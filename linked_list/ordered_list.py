"""Class definitions and helper functions for a Linked List
Author: Ben Paulson
"""

def add_helper(item, next_node, prev_node):
    ...
    """
    if item < next_node.val:
        new_node = Node(item, next_node, prev_node)
        next_node.prev = new_node
        prev_node.next_elem = new_node
        return None
    return add_helper(item, next_node.next_elem, next_node)
...
class OrderedList:
    ...
    def remove(self, item):
        ...
        if self.num_items == 1 and self.head.val == item:
            self.head = None
            self.tail = None
            self.num_items = 0
            return 0
    ...
    def pop(self, pos=None):
        ...
        if pos is not None:
            ...
            if direction > 0:
                val = pop_helper(pos, self.head, 0, direction)
                self.num_items -= 1
                return val
            val = pop_helper(pos, self.tail, self.num_items - 1, direction)
            self.num_items -= 1
            return val
        if self.num_items == 0:
            raise IndexError("pop index out of range")
...
class Node:
    ...
    """
    def __init__(self, val, next_elem, prev):
        self.val = val
    ...
    def __repr__(self):
        return f"Node({self.val}, {self.next_elem})"
